...
currentLabels           = [allLabels(i), allLabels(j)];
% indexes of the windows that needed for the classification
tempInds                = ((labelsAll == allLabels(i)) | (labelsAll == allLabels(j)));
...
res_soft_vote           = zeros(2,2,participantsNum); % added by hilal
info                    = cell(1, participantsNum); 
notTested               = [];
plotsNum                = ceil(participantsNum/2);
tic
close all; figure(1);  
% run leave one out
for k = 1:participantsNum
    ...
    method              = 1; % 0 - do nothing; 1 - over sample smallest group; 2 - under sample biggest group
    dropFromTraining    = 0; % randomly remove x% of data from training (0 means dont remove)
    [trainData, trainDegreeLabels, trainParticipantLabels] = balanceData(trainData, trainDegreeLabels, trainParticipantLabels, allLabels, i, j, method);
...
SR =printResluts(res_mvote,fnames{i},fnames{j});

fprintf('\n \nResults using soft vote \n')
printResluts(res_soft_vote,fnames{i},fnames{j})
