function net = helperImportMatConvNet2(matfile)
...
for i = 1:numLayers
    if isstruct(matconvnet.layers)
        l = matconvnet.layers(i);
    elseif iscell(matconvnet.layers)
        l = matconvnet.layers{i};
    end
    
    if ~isnan(strfind(l.type,'dagnn.'))
        type = lower(l.type(7:end));
    end
    ...
    switch type
        ...
        case 'softmax'                                                                                       
            ...
            outputSize = layers{end}.OutputSize(end);
            layers{end+1} = nnet.internal.cnn.layer.Softmax(l.name);
            
            layers{end+1} = nnet.internal.cnn.layer.CrossEntropy('classificationLayer', outputSize);
...
if isfield(n,'averageImage')
    layer = nnet.internal.cnn.layer.ImageInput('input', imageSize, normalizations, []);
    layer.AverageImage = single(n.averageImage);
else
    layer = nnet.internal.cnn.layer.ImageInput('input', imageSize, [], []);
end
